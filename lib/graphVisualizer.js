...
var Attributes = __webpack_require__(0)
...
var dotparser = __webpack_require__(4);
...
/* harmony export (immutable) */ __webpack_exports__["GraphVisualizer"] = GraphVisualizer;
...
var Context = __webpack_require__(1)
var Attributes = __webpack_require__(0)
var AttributeParser = __webpack_require__(8)
var Shapes = __webpack_require__(9)
...
module.exports = class Parser
{
    ...
    static ParseSubgraph(context, element, nodes)
    {
        ...
        */
        Parser.FixSubgraphs(element);
        var newContext = context.Copy();
        element.children.filter(e => e.type == "attr_stmt").forEach(e => Parser.ParseGraphAttributes(newContext, e));
        Parser.DecorateGraph(element, newContext);
        element.children.filter(e => e.type == "subgraph").forEach(e => {
            newContext.container = context.container.group();
            newContext.isRoot = false;
            Parser.ParseSubgraph(newContext, e, nodes);
        });
        element.children.filter(e => e.type == "edge_stmt").forEach(e => Parser.BuildEdge(newContext, e));
        element.children.filter(e => e.type == "node_stmt").forEach(e => Parser.ParseNode(newContext, e, nodes));
    }
    ...
    static ParseNode(context, node, nodes)
    {
        ...
        var group = context.container.group().id(attr.id ? attr.id : Parser.ClassPrefix() + node.node_id.id);
        var shape = Parser.ParseShape(group, attr, node.node_id.id);
    ...
    static ParseGraphAttributes(context, attribute)
    {
        ...
        if (context.isRoot && attribute.target == "graph") {
            Parser.SetupDocumentBounds(context, attributes);
        }
    }
    ...
    static ParseShape (container, attributes, id)
    {
        ...
        if (shapeAtr === "record") {
            return Parser.ParseRecord(container, attributes);
        } else {
            var pos = AttributeParser.ParseNodePosition(attributes.pos);
            var shape = Parser.BuildShape(shapeAtr, container, attributes);
    ...
    static BuildEdgeTip(doc, start, end)
    {
        if (end.type != 'e') {
            return;
        }
    ...
    static BuildEdge(context, edge)
    {
        var attr = context.edgeDefaults.Copy();
        attr.Override(edge.attr_list);
        var positions = AttributeParser.ParsePositionArray(attr.pos);
        var data = Parser.BuildSplines(positions);
        var defaults = Attributes.Default;
        var group = context.container.group().id(attr.id ? attr.id : Parser.ClassPrefix() + edge.edge_list.map((c, i, a) => { return c.id}).join('-'));
        ...
        Parser.BuildEdgeTip(group, positions[positions.length - 1], positions[0]);
        if (attr.label) {
